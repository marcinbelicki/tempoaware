package pl.belicki.tempoaware.info
...
object Info {
  ...
  implicit def iorTNecFromFuture[T](future: Future[T])(implicit
      ec: ExecutionContext
  ): IorTNec[T] =
    IorT.liftF[Future, NonEmptyChain[Info], T](future)

  implicit def iorTNecFromFutureIor[T](
      future: Future[Ior[NonEmptyChain[Info], T]]
  ): IorTNec[T] =
    IorT(future)

  implicit def flatMap(implicit ec: ExecutionContext): FlatMap[IorTNec] =
    new FlatMap[IorTNec] {
      ...
      override def flatMap[A, B](fa: IorTNec[A])(
          f: A => IorTNec[B]
      ): IorTNec[B] = innerFlatMap.flatMap(fa)(f)

      override def tailRecM[A, B](a: A)(
          f: A => IorTNec[Either[A, B]]
      ): IorTNec[B] = innerFlatMap.tailRecM(a)(f)

      override def map[A, B](fa: IorTNec[A])(f: A => B): IorTNec[B] =
        innerFlatMap.map(fa)(f)
        ...
        val value = for {
          ...
          case Ior.Left(a) =>
            valueB match {
              ...
              case Ior.Both(leftB, _) => Ior.left(a.combine(leftB))
