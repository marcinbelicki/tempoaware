...
class EndTime[V](
  ...
  protected def transformString(value: String): IorNecChain[LocalTime] =
    try Ior.right(Chain(LocalTime.parse(value)))
      ...
      case parseException: DateTimeParseException => Ior.leftNec(Info(s"Could not read end-time from ${parseException.getParsedString}, message: ${parseException.getMessage}", InfoType.Error))
    }
}
