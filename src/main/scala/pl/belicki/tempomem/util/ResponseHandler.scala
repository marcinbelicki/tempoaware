...
object ResponseHandler {
      ...
      future.flatMap {
        response =>
          pf
            ...
            .getOrElse(Future.failed(new IllegalStateException(s"Unexpected status code ${response.status} with body ${response.body[String]}.")))
      }
  ...
  def unauthorizedAndNotFound(url: String): PartialFunction[Int, StandaloneWSRequest#Response => Future[StandaloneWSRequest#Response]] = {
    ...
    case 401 => response => Future.failed(new IllegalStateException(s"Request was unauthorized - status code 401 for url $url; response body: ${response.body[String]}"))
    case 404 => response => Future.failed(new IllegalStateException(s"Resource not found - status code 404 for url $url; response body: ${response.body[String]}"))
  }
